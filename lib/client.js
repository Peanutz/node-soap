...
Client.prototype._setSequenceArgs = function(argsScheme, args) {
  ...
  for (var partIndex in argsScheme) {
    if(typeof args[partIndex] === 'undefined') {
      ...
      if (_.isArray(args[partIndex]) && typeof args[partIndex][0] !== 'object') {
        for (var subPart in argsScheme[partIndex]) {
          if (subPart.slice(-2) === '[]' && typeof argsScheme[partIndex][subPart] !== 'object') {
            result[partIndex] = args[partIndex];
          }
        }
      } else {
        if (!_.has(result, partIndex)) {
